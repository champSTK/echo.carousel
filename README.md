...
```bash
git clone https://github.com/champSTK/echo.carousel.git
cd echo.carousel
npm install
```
...
| Action | Result |
|--------|--------|
| **Tap a card** | Play that track |
| **Hold a card (0.5s)** | Open file picker to upload audio |
| **Tap the central video** | Open file picker to upload a new video |
| **Scroll (desktop)** | Rotate the carousel |
| **Swipe horizontally (mobile)** | Rotate the carousel |
| **🔁 button** | Toggle queue loop / single track loop |
| **🔊 button** | Mute / unmute |
| **💾 button** | Save all uploads to IndexedDB (persists after refresh) |
| **Wait 5 seconds** | App enters sleep / idle mode |
| **Any interaction** | Wake from sleep mode |
...
| 3D rendering | CSS `transform-style: preserve-3d` |
